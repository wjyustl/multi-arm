...
class RobotTaskEnv(gym.Env):
    ...
    def __init__(self, robot: PyBulletRobot, task: Task) -> None:
        ...
        obs = self.reset()
        observation_shape = np.zeros(3).shape
        achieved_goal_shape = np.zeros(3).shape
        desired_goal_shape = np.zeros(3).shape
        ...
        desired_goal = gym.spaces.Box(-10.0, 10.0, shape=achieved_goal_shape, dtype=np.float32)
        achieved_goal = gym.spaces.Box(-10.0, 10.0, shape=desired_goal_shape, dtype=np.float32)
        self.observation_space = [observation, desired_goal, achieved_goal]
    ...
    def step(self, action: np.ndarray) -> Tuple[Dict[str, np.ndarray], float, bool, Dict[str, Any]]:
        ...
        obs = np.array(obs[0])
        info = {"is_success": self.task.is_success(obs, self.task.get_goal())}
        reward = self.task.compute_reward(obs, self.task.get_goal(), info)
        assert isinstance(reward, float), "the reward must be float,but got {0}".format(
            type(reward))  # needed for pytype cheking

        reward = reward.reshape(1, 1)

        done = info["is_success"].reshape(1, 1)
        assert type(obs) != list, "Here !!!!!!"
